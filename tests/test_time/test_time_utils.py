...
class TestDateCorrectnessCaretaker:
    ...
    def test__are_both_day_and_week_in_self_or_other_nones(self, tdcc):
        ...
        assert tdcc._are_week_and_day_nones_in_one(t1, t2) is True
        t1 = Time(10, 20, Day.MONDAY, Week(1))
        ...
        t1 = Time(10, 20, Day.MONDAY, Week(1))
        t2 = Time(11, 20, Day.MONDAY, Week(1))
        assert tdcc._are_week_and_day_nones_in_one(t1, t2) is False
    ...
    def test_assert_initial_arguments_day_and_week_correct__ok(self, tdcc):
        tdcc.assert_args_day_and_week_correct(None, None)
        for day in DAYS:
            tdcc.assert_args_day_and_week_correct(day, Week(1))

    def test_assert_days_and_weeks_correctness__incorrect_type(self, tdcc):
        with pytest.raises(RuntimeError):
            tdcc.assert_args_day_and_week_correct(1, Week(1))
        with pytest.raises(RuntimeError):
            tdcc.assert_args_day_and_week_correct(Day.MONDAY, 1)
        with pytest.raises(RuntimeError):
            tdcc.assert_args_day_and_week_correct(2, 1)

    def test_assert_days_and_weeks_correctness__incorrect__not_both_set_or_none(self, tdcc):
        with pytest.raises(RuntimeError):
            tdcc.assert_args_day_and_week_correct(None, Week(1))
        with pytest.raises(RuntimeError):
            tdcc.assert_args_day_and_week_correct(Day.MONDAY, None)
