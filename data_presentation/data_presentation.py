...
def generate_pdfs(groups, lecturers, rooms, name):
    ...
    for name, items in names_items:
        generate_pdfs_for_items(result_path, name, items)


def generate_pdfs_for_items(path: Path, name, items: List[WithSchedule]):
    ...
    for item in items:
        ...
        save_yaml(file_path, yaml_txt)
        save_pdf(file_path)
        delete_yaml(file_path)
